 ...
 * #######  断点续传  ######
 */
var host = "ws://localhost:8080/WebSocketForJavaEE7/websocket/web-client";
...
function webSocketConn() {
	try {
		...
		websocket.onopen = function() {
			msg.innerHTML += "<p>Socket状态： " + readyState[websocket.readyState]
					+ "</p>";
		};
		websocket.onmessage = function(event) {
			...
			if (event.data.indexOf("ok") != -1 && uploadState == 1) {
				if (endSize == file.size) {
					localStorage.removeItem(file.lastModifiedDate + "_"
							+ file.name);
...
function getLastestUploadEndSize(uploadFile) {
	var lastestLen = localStorage.getItem(uploadFile.lastModifiedDate + "_"
			+ uploadFile.name);
	if (lastestLen) {
